...
extension AppDelegate : UNUserNotificationCenterDelegate{
    ...
    func receiveRemoteNotificationForbackground(userInfo : [AnyHashable : Any]){
        ...
        let tempDic = userInfo["aps"] as! [String : Any]
        let alertS = tempDic["alert"] as! String
        ...
        let alertController = UIAlertController(title: "新信息提醒",
                                                message: alertS, preferredStyle: .alert)
 ...
 extension AppDelegate {
    ...
    private func dealAlipay(resultDic: [AnyHashable: Any]?) {
        ...
        if status == "9000"{
            let not = Notification.init(name: NSNotification.Name.init(ALIPAY_SUCCESS), object: nil, userInfo: nil)
            NotificationCenter.default.post(not)
