...
//    let searchBtn = homeSearchButton()
    ...
    lazy var noticeWordV : LEDWordsView = {
        let space = AppDelegate.shareIntance.space
        let l = LEDWordsView.init(frame: CGRect.init(x: 0, y: space.topSpace + 44, width: SCREEN_WIDTH, height: 30))
        l.naviVC = self.navigationController
        return l
    }()
    ...
    func setupNavibar(){
        ...
        let qrcodeBtn = imageButton.init(frame: CGRect.init(x: 6, y: 10, width: 22, height: 22))
        qrcodeBtn.addTarget(self, action: #selector(HomeTableViewController.qrcodeVC), for: .touchUpInside)
        qrcodeBtn.imgV.image = UIImage.init(named: "qrcodeBlack")
        let contV = UIView.init(frame: CGRect.init(x: 0, y: 0, width: 44, height: 44))
        contV.addSubview(qrcodeBtn)
    ...
    func refreshView(layer : CGFloat){
        ...
        selectV.frame = CGRect.init(x: 0, y: ScrollImageVHeight + FuncSizeWidth * layer + ViewGap, width: SCREEN_WIDTH, height: SelectViewHeight)
        gooodnewsV.frame = CGRect.init(x: 0, y: ScrollImageVHeight + FuncSizeWidth * layer + ViewGap * 2 + SelectViewHeight, width: SCREEN_WIDTH, height: GoodnewsHeight)
        knowledgeVC.view.frame = CGRect.init(x: 0, y: ScrollImageVHeight + FuncSizeWidth * layer + SelectViewHeight + GoodnewsHeight + ViewGap * 3, width: SCREEN_WIDTH, height: KnownledgeViewHeight)
        
        let headV = tableV.tableHeaderView
        headV?.frame = CGRect.init(x: 0, y: 0, width: SCREEN_WIDTH, height: ScrollImageVHeight + FuncSizeWidth * layer + SelectViewHeight + ViewGap * 2 + KnownledgeViewHeight)
    ...
    func requestData(){
        ...
        HttpRequestManager.shareIntance.HC_notice { [weak self](arr, s) in
            if let modelArr = arr{
                self?.view.insertSubview((self?.noticeWordV)!, aboveSubview: (self?.tableV)!)
                self?.noticeWordV.modelArr = modelArr
            }else{
            }
            group.leave()
        }
