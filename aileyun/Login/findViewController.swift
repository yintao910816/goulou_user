...
class findViewController: UIViewController {
    ...
    func initFindPasswordV(){
        ...
        verifyBtn.setTitle("获取验证码", for: UIControl.State.normal)
        verifyBtn.titleLabel?.font = UIFont.init(name: kReguleFont, size: 13)
        verifyBtn.backgroundColor = kDefaultThemeColor
        verifyBtn.layer.cornerRadius = 5
        
        verifyBtn.addTarget(self, action: #selector(findViewController.startCount), for: UIControl.Event.touchUpInside)
        ...
        seeBtn.setImage(UIImage.init(named: "显示"), for: UIControl.State.normal)
        
        seeBtn.addTarget(self, action: #selector(findViewController.passwordStyle), for: UIControl.Event.touchUpInside)
        ...
        resetPasswordBtn.setTitle("重置密码", for: UIControl.State.normal)
        resetPasswordBtn.layer.cornerRadius = 10
        resetPasswordBtn.backgroundColor = kDefaultThemeColor
        
        resetPasswordBtn.addTarget(self, action: #selector(findViewController.resetPassword), for: UIControl.Event.touchUpInside)
...
//        }
    ...
    func showSecond(){
        ...
        if count == KMaxSeconds {
            ...
            verifyBtn.setTitle(showString, for: UIControl.State.normal)
    ...
    @objc func passwordStyle(){
        ...
        if passwordTF.isSecureTextEntry {
            seeBtn.setImage(UIImage.init(named: "隐藏"), for: UIControl.State.normal)
        }else{
            seeBtn.setImage(UIImage.init(named: "显示"), for: UIControl.State.normal)
        }
    }
