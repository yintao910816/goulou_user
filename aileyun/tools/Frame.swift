...
struct LayoutSize {
    ...
    static func height(containsTop viewController: UIViewController) ->CGFloat {
        return PPScreenH - UIApplication.shared.statusBarFrame.height - (viewController.navigationController?.navigationBar.frame.size.height ?? 0)
    }
    
    static func height(containsTopAndBottom viewController: UIViewController) ->CGFloat {
        return PPScreenH - UIApplication.shared.statusBarFrame.height - (viewController.navigationController?.navigationBar.frame.size.height ?? 0) - (viewController.tabBarController?.tabBar.frame.size.height ?? 0)
    }
    
    static func bottomHeight(_ viewController: UIViewController) ->CGFloat {
        return viewController.tabBarController?.tabBar.frame.size.height ?? 0
    }
