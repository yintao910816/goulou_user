...
class CommentStarView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        for i in 0..<5 {
            ...
            starBtn.tag = i + 1
            starBtn.setImage(UIImage.init(named: "评星2"), for: UIControl.State.normal)
            starBtn.setImage(UIImage.init(named: "评星1"), for: UIControl.State.selected)
            
            starBtn.addTarget(self, action: #selector(CommentStarView.chooseAction), for: UIControl.Event.touchUpInside)
