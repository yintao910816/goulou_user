...
class WebViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let rightItem = UIBarButtonItem.init(image: UIImage.init(named: "分类"), style: UIBarButtonItem.Style.plain, target: self, action: #selector(WebViewController.menu))
...
extension WebViewController : UIWebViewDelegate{
    ...
    func webViewDidFinishLoad(_ webView: UIWebView){
        ...
        // 掉起支付
        let payConfirmApp: @convention(block) () ->() = { [weak self] in
            self?.requestPayInfo(jsParams: JSContext.currentArguments())
        }
        context?.setObject(unsafeBitCast(payConfirmApp, to: AnyObject.self), forKeyedSubscript: "payConfirmApp" as NSCopying & NSObjectProtocol)
...
extension WebViewController { // 支付
    ...
    fileprivate func requestPayInfo(jsParams: [Any]?) {
        ...
        for idx in 0..<params.count {
            if idx == 0 {
                token = ((params[0] as AnyObject).toString()) ?? ""
            }else if idx == 1 {
                orderID = ((params[1] as AnyObject).toString()) ?? ""
            }
        }
